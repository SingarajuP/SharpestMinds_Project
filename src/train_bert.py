...
predicted_labels = predicted_results.predictions.argmax(
    -1
)  # Get the highest probability prediction
predicted_labels = (
    predicted_labels.flatten().tolist()
)  # Flatten the predictions into a 1D list

print(classification_report(dataset["test"]["label"], predicted_labels))

confusion_matrix(dataset["test"]["label"], predicted_labels)
